...
const GlassInputWrapper = ({ children }: { children: React.ReactNode }) => (
  <div className="rounded-2xl border border-purple-500/20 bg-black/40 backdrop-blur-sm transition-colors focus-within:border-purple-400/70 focus-within:bg-purple-500/10">
    {children}
  </div>
);

const TestimonialCard = ({ testimonial, delay }: { testimonial: Testimonial, delay: string }) => (
  <div className={`animate-testimonial ${delay} flex items-start gap-3 rounded-3xl bg-black/40 backdrop-blur-xl border border-purple-500/20 p-5 w-64`}>
...
export const SignInPage: React.FC<SignInPageProps> = ({
  ...
  description = "Sign in to access your content control center",
  heroImageSrc,
  testimonials = [],
  onSignIn,
  onGoogleSignIn,
      ...
      <section className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          <div className="flex flex-col gap-6">
            <h1 className="animate-element animate-delay-100 text-4xl md:text-5xl font-semibold leading-tight bg-gradient-to-r from-white via-purple-200 to-purple-400 bg-clip-text text-transparent">{title}</h1>
            ...
            <form className="space-y-5" onSubmit={onSignIn}>
                ...
                <GlassInputWrapper>
                  <div className="relative">
                    ...
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-3 flex items-center">
                      {showPassword ? <EyeOff className="w-5 h-5 text-gray-400 hover:text-purple-400 transition-colors" /> : <Eye className="w-5 h-5 text-gray-400 hover:text-purple-400 transition-colors" />}
                    </button>
                  </div>
                </GlassInputWrapper>
              </div>
              ...
              <div className="animate-element animate-delay-500 flex items-center justify-between text-sm">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" name="rememberMe" className="w-4 h-4 rounded border-purple-500/50 bg-black/40 checked:bg-purple-600" />
              ...
              <button type="submit" className="animate-element animate-delay-600 w-full rounded-2xl bg-gradient-to-r from-purple-600 to-purple-800 py-4 font-medium text-white hover:from-purple-700 hover:to-purple-900 transition-all shadow-lg shadow-purple-500/50">
                Sign In
              </button>
            </form>
            ...
            <button onClick={onGoogleSignIn} className="animate-element animate-delay-800 w-full flex items-center justify-center gap-3 border border-purple-500/20 rounded-2xl py-4 hover:bg-purple-500/10 transition-colors text-white">
                <GoogleIcon />
